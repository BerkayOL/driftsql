...
@DriftAccessor(tables: [FloorsTable, BuildingsTable])
class FloorDao extends DatabaseAccessor<AppDatabase> with _$FloorDaoMixin {
  ...
  Stream<List<FloorWithBuilding>> watchFloorsWithBuilding(
    int buildingId,
  ) {
    final query = select(floorsTable).join([
      innerJoin(
        buildingsTable,
